...
def sliding_predict(model, image, num_classes, flip=True):
    ...
    tile_size = (int(image_size[2]//3), int(image_size[2]//3))
    overlap = 1/3
...
def parse_arguments():
    ...
    parser.add_argument('-c', '--config', default='saved/UNet/09-26_16-38/config.json',type=str,
                        help='The config used to train the model')
    parser.add_argument('-mo', '--mode', default='sliding', type=str,
                        help='Mode used for prediction: either [multiscale, sliding]')
    parser.add_argument('-m', '--model', default='saved/UNet/09-26_16-38/best_model.pth', type=str,
                        help='Path to the .pth model checkpoint to be used in the prediction')
    parser.add_argument('-i', '--images', default='Massachusetts/images', type=str,
